...
void compute_energy_matrix(const Image* img, Matrix* energy) {
  ...
  int wid = Image_width(img);
  int heit = Image_height(img);
  int temp = 0;
  int maximum = 0;
  for (int row=1; row<heit-1;row++) {
    for (int col=1; col<wid;col++) {
      ...
      *Matrix_at(energy,row,col) = squared_difference(n,s) + squared_difference(w,e);
      if (*Matrix_at(energy,row,col>temp)) {
        temp = *Matrix_at(energy,row,col);
        maximum = *Matrix_at(energy,row,col);
      }
    }
  }
...
int wid = Matrix_width(energy);
int heit = Matrix_height(energy);
for (int col=0;col<wid;col++) {
  *Matrix_at(cost,0,col) = *Matrix_at(energy,0,col);
}

for (int row=1;row<heit;row++) {
  *Matrix_at(cost,row,0) = *Matrix_at(energy,row,0) + Matrix_min_value_in_row(cost,row-1,0,1);
}

for (int row=1;row<heit;row++) {
  *Matrix_at(cost,row,(wid-1)) = *Matrix_at(energy,row,(wid-1)) + Matrix_min_value_in_row(cost,row-1,(wid-2),(wid-1));
}
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  int wid = Matrix_width(cost);
  int heit = Matrix_height(cost);
  int min_col = 0;
  int col_start = 0;
  int col_end = (wid-1);
  for (int row=(heit-1);row>=0;row--) {
    min_col = Matrix_column_of_min_value_in_row(cost,row,col_start,col_end);
    seam[row] = min_col;
    if (min_col != 0 && min_col != (wid-1)) {
      col_start = min_col-1;
      col_end = min_col+1;
    }
    else if (min_col == 0) {
